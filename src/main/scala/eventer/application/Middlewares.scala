...
class Middlewares[SessionServiceR](jwts: Jwts) extends Http4sDsl[Task] with Codecs[Task] {
...
//  type AuthF[A] = RIO[SessionServiceR with Clock, A]
  ...
  def auth: AuthMiddleware[Task, SessionUser] = AuthMiddleware.noSpider(authUser, _ => Forbidden())
